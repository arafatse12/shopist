...
class LoginController extends Controller
{
  ...
  public function goToAdminLoginPage(){
    ...
    if(Cookie::has('remember_me_data')){
      $userDetails        =     User::find(Cookie::get('remember_me_data')[0]);
      $password           =     bcrypt(Cookie::get('remember_me_data')[1]);

      if(Hash::check(Cookie::get('remember_me_data')[1], $password) && Hash::check(Cookie::get('remember_me_data')[1], $userDetails['password'])){
        $user_view = $userDetails['email'];
        $pass_view = Cookie::get('remember_me_data')[1];
      }
    }
  ...
  public function goToFrontendLoginPage(){
    ...
    if(Cookie::has('frontend_remember_me_data')){
      $userDetails        =     User::find(Cookie::get('frontend_remember_me_data')[0]);
      $password           =     bcrypt(Cookie::get('frontend_remember_me_data')[1]);

      if(Hash::check(Cookie::get('frontend_remember_me_data')[1], $password) && Hash::check(Cookie::get('frontend_remember_me_data')[1], $userDetails['password'])){
        $user_view = $userDetails['name'];
        $pass_view = Cookie::get('frontend_remember_me_data')[1];
      }
    }
    ...
    return view('pages.auth.user-login')->with($get_data);
  }
        ...
        if(!empty($data) && isset($data->password) && isset($data->id)){
          ...
          if(Hash::check(Input::get('admin_login_password'), $password) && Hash::check(Input::get('admin_login_password'), $data->password)){
            ...
            if($remember == TRUE){
              ...
              array_push($remember_data, $data->id);
              array_push($remember_data, Input::get('admin_login_password'));
        ...
        if(!empty($data) && isset($data->password) && isset($data->id)){
          ...
          if(Hash::check(Input::get('login_password'), $password) && Hash::check(Input::get('login_password'), $data->password)){
            ...
            if($remember == TRUE){
              ...
              array_push($cookieData, $data->id);
              array_push($cookieData, Input::get('login_password'));
